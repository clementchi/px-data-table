...
document.addEventListener("WebComponentsReady", function() {
  ...
  multiSelectTest = document.getElementById('multi-select-test');
  multiSelectTest.tableData = minidata;
...
function runTests() {
 ...
 suite('Column show/hide tests', function(){
    ...
    suite('multi-selectable table test', function(){
      ...
      test('should select all rows when clicking select all', function(done){
        var selectAllCheckbox = fixture.querySelector('#selectAllCheckbox');
        selectAllCheckbox.click();
        flush(function(){
          var checkedRows = fixture.querySelectorAll('.tr.rows .td input[type="checkbox"]:checked');
          assert.equal(checkedRows.length, 10);
          assert.equal(testUtil.getElementText('#multi-select-test .column-head:nth-child(1)'), 'Selected (10)');
          assert.equal(fixture.selectedRows.length, 10);
          done();
        });
      });

      test('should uncheck the select all when 1 row is not checked', function(done){
        var selectAllCheckbox = fixture.querySelector('#selectAllCheckbox');
        var tableRowCheckboxes = fixture.querySelectorAll('.tr.rows .td input[type="checkbox"]');
        assert.equal(selectAllCheckbox.checked,true);

        tableRowCheckboxes[0].click();
        tableRowCheckboxes[2].click();
        tableRowCheckboxes[4].click();

        flush(function(){
          var checkedRows = fixture.querySelectorAll('.tr.rows .td input[type="checkbox"]:checked');
          assert.equal(selectAllCheckbox.checked,false);
          assert.equal(checkedRows.length, 7);
          assert.equal(testUtil.getElementText('#multi-select-test .column-head:nth-child(1)'), 'Selected (7)');
          assert.equal(fixture.selectedRows.length, 7);
          done();
        });
      });

      test('should check the select all box when all rows on display are checked', function(done){
        //check for selected all checkbox state initial state
        var selectAllCheckbox = fixture.querySelector('#selectAllCheckbox');
        var tableRowCheckboxes = fixture.querySelectorAll('.tr.rows .td input[type="checkbox"]');
        assert.equal(selectAllCheckbox.checked,false);
      ...
      test('should not have the check box when is-selectable-function-name is used', function(done){
        var fixtureCustom = document.querySelector('#custom-multi-select-test');
        //check for selected all checkbox state initial state
        var selectAllCheckbox = fixtureCustom.querySelector('#selectAllCheckbox');
        var tableRowCheckboxes = fixtureCustom.querySelectorAll('.tr.rows .td input[type="checkbox"]');
        assert.equal(selectAllCheckbox.checked,false);
        assert.equal(tableRowCheckboxes.length,1);

        tableRowCheckboxes[0].click();

        flush(function(){
          var checkedRows = fixtureCustom.querySelectorAll('.tr.rows .td input[type="checkbox"]:checked');
          assert.equal(selectAllCheckbox.checked,true);
          assert.equal(checkedRows.length, 1);
          assert.equal(testUtil.getElementText('#custom-multi-select-test .column-head:nth-child(1)'), 'Selected (1)');
    ...
    suite('server table test',function(){
      ...
      var assertTableServerState=function(eventData,firstItemIndex,sortedColumn,descending,pageSize){
        assert.equal(eventData.firstItemIndex, firstItemIndex);
        assert.equal(eventData.sortedColumn, sortedColumn);
        assert.equal(eventData.descending, descending);
        assert.equal(eventData.pageSize, pageSize);
      };
      var fixture;
      suiteSetup(function(){
        fixture = document.querySelector('#server');
      });
      ...
      test ('should not sort',function(done){
        ...
        flush(function(){
          ...
          assert.include(lastNameRow[6].textContent, 'Bartlett');
          done(); // end the test
        });
      });
      ...
      test ('should not paginate',function(done){
        var page2Button = serverPagination.querySelector(page2Selector);
        ...
        //before page 2 is clicked, row should be 20
        assert.equal(allLastNameRow.length, '20');
        page2Button.click();
        setTimeout(function(){
          assert.equal(allLastNameRow.length, '20');
          assertTableServerState(eventData, '21', 'first', true, 20);
          done();
        },15);
      });

      test('should emit table state changed event when sort on column',function(done){
        var emailHeaderSelector = '.aha-email-th > div > span';
        var emailHeader = fixture.querySelector(emailHeaderSelector);
        emailHeader.click();
        setTimeout(function(){
          assertTableServerState(eventData, '1', 'email', false, 20);
          done();
        },10);
      });

      test('should emit table state changed event when clicking on page number',function(done){
        var page1Selector = '.paging.style-scope.px-pagination > span > :nth-child(1)';
      ...
      test('should emit table state changed event when changing page size',function(done){
        ...
        setTimeout(function(){
          ...
          assertTableServerState(eventData, '1', 'email', false, 10);
          done();
        },10);
      });
  });
